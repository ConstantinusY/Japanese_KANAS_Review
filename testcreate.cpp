    ...
    while (usedItemNum < (int)items.size()) {
        ...
        if(usedItemNum >= (int)items.size()-kanaNumPerLine)  {
            for(int i = usedItemNum ; i <  (int)items.size() ; i++)  {
                result += to_string(usedItemNum + 1);
                result += ". ";
                result += items.at(usedItemNum);
        ...
        while (usedItemNum < (int)items.size()) {
            ...
            if(usedItemNum >= (int)items.size()-kanaNumPerLine)  {
                for(int i = usedItemNum ; i <  (int)items.size() ; i++)  {
                    result += to_string(usedItemNum + 1);
                    result += ". ";
                    result += kanas.find(items.at(usedItemNum))->second;
                    result += "\t";
                    usedItemNum++;
                }
                break;
    ...
    ui(new Ui::TestCreate)
{
    ...
    connect(ui->chooseFile,&QPushButton::clicked,[=](){
        this->filePath = QFileDialog::getOpenFileName(this,"选择文件",".","*.txt  |  *.doc  |  *.docx").toUtf8().data();
    ...
    connect(ui->create,&QPushButton::clicked,[=](){ //当按下生成按钮进行一系列判断
        ...
        ui->outPutArea->setText(QString::fromStdString(test));
